...
const Homeownership: React.FC = () => {
  const [elementHeight1, setElementHeight1] = useState<number | undefined>(undefined);
  const elementRef1 = useRef<HTMLDivElement>(null)

  const updateHeight1 = () => {
    if (elementRef1.current) {
      setElementHeight1(elementRef1.current.offsetHeight)
    }
  }

  useEffect(() => {
    updateHeight1();
    window.addEventListener('resize', updateHeight1)
    return () => {
      window.removeEventListener('resize', updateHeight1)
    }
  }, [])
  ...
  return (
    <div className="py-20 border-t-2 flex flex-col justify-center items-center">
      ...
      <div className="mt-20 lg:flex justify-center items-center p-5">
        <div className="basis-1/2 p-16 grid gap-4 bg-black bg-opacity-5 rounded-t-lg lg:rounded-l-lg " ref={elementRef1}>
        ...
        <div className="basis-1/2 h-full">
          <Image
            ...
            style={{height: `${elementHeight1}px`}}
          />
        </div>
      </div>
    </div>
